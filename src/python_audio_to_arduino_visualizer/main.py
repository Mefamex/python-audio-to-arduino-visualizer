...
from python_audio_to_arduino_visualizer.ui import WIDTH, cyan, dim, fail, green, header, hint_for, red, selection_box, step
...
def _classify(error: BaseException | object | None) -> str:
    ...
    if any(key in text for key in ("could not open port", "/dev/tty", "ttyusb", "ttyacm", "serial", "arduino", "write failed", "input/output error")): return "NO ARDUINO connection found"
    if any(key in text for key in ("parec", "pulseaudio", "audio stream")): return "NO AUDIO source found"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sample-rate", type=int, default=config.SAMPLE_RATE, help="Audio sample rate (default: 16000)")
    parser.add_argument("--chunk-size", type=int, default=config.CHUNK_SIZE, help="Audio chunk size (default: 300)")
    parser.add_argument("--baud-rate", type=int, default=config.BAUD_RATE, help="Arduino serial baud rate (default: 115200)")
...
def run(port: str, device: str, sample_rate: int, chunk_size: int, baud_rate: int, printDebug: bool = True) -> None:
    line = cyan("=" * WIDTH)
    if printDebug:
        print("\nConnecting...")
        print(f"  • Port:   {port}")
    ...
    try:
        with serial.Serial(port, baud_rate, timeout=0.2) as arduino:
            ...
            if process.stdout is None: raise RuntimeError("parec failed to create an audio stream.")
            analyzer = AudioAnalyzer(sample_rate=sample_rate, chunk_size=chunk_size)

            # Always announce success (retries stay silent until they reconnect).
            print("")
            print(line)
            print(f" Connecting to port {port}...")
            print(f" {green('✔')} Arduino connection successful!")
            print(f" {green('✔')} Starting PulseAudio (parec)...")
            print(f" {green('✔')} Audio-reactive LEDs are running. Press Ctrl+C to exit.")
            ...
            while True:
                ...
                if len(raw_data) < chunk_size * 2: raise RuntimeError("parec closed the audio stream.")
...
def main() -> None:
    ...
    while True:
        ...
        try:
            # Only the first attempt shows the full banner; retries stay compact.
            run(port, device, args.sample_rate, args.chunk_size, args.baud_rate, printDebug=first_failure is None)
            break
        except KeyboardInterrupt:
            print("\n[INFO] User requested exit (Ctrl+C). Shutting down..."); sys.exit(0)
        except (Exception, SystemExit) as e:
            # Ignore successful manual exits
            if isinstance(e, SystemExit) and e.code == 0: sys.exit(0)
            # Stable for a while -> treat the next failure as a fresh outage.
            if time.time() - start_time > 10: first_failure = None
            is_first = first_failure is None
            if first_failure is None: first_failure = time.time()
            ...
            if orig is None and isinstance(e, Exception): orig = e
            label = _classify(orig)

            if is_first:
                # Full detail only once per outage; retries stay compact.
                sys.stdout.flush()
                if orig is not None: print(f"✖ Error: {orig}", file=sys.stderr)
                print(f"{red('=' * WIDTH, sys.stdout)}")
            print(f"{_now()} [ERROR] {label} (elapsed {int(elapsed)}s / {RETRY_TIMEOUT}s)")
