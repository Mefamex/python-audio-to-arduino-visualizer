...
def list_ports() -> list[str]:
    ...
    return sorted(port.device for port in ports if "ttyU" in port.device or "ttyA" in port.device)
...
def choose_port() -> str:
    ports = list_ports()
    if not ports: raise RuntimeError("No Arduino serial ports found.")
...
def init_port(args_port: str) -> str:
    if args_port and check_port(args_port): return args_port
    ports = list_ports()
    if not ports: raise RuntimeError("No Arduino serial ports found.")
    if len(ports) == 1: return ports[0]
    return choose_port()
